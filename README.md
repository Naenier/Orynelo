...
OpsDoctor is pre-1.0 software under active development. The report schema is
versioned, but commands and storage migrations may still change before the
first stable release. Linux, with Ubuntu 26.04 LTS as the primary target, is
the development platform. CI is configured to compile explicitly on Ubuntu
26.04 LTS as well as macOS and Windows; those workflows must pass on the
published repository before macOS or Windows should be treated as fully
supported.

No screenshots are included because the repository does not yet contain
verified release screenshots.
...
```bash
opsdoctor diagnose https://example.com
opsdoctor diagnose git.example.internal:22
opsdoctor diagnose https://example.com --ip-version 4
opsdoctor diagnose https://example.com --format json
opsdoctor diagnose https://example.com --format markdown --output report.md
opsdoctor version
opsdoctor version --json
opsdoctor completion bash
```
...
go build -o bin\opsdoctor-desktop.exe .\cmd\opsdoctor-desktop
```

Native Fyne build dependencies are still required for the desktop binary.
Release builds inject version, commit, and build date through linker flags;
source files are never rewritten to record a build version.
...
```bash
docker run --rm ghcr.io/naenier/opsdoctor:latest \
...
| --- | --- | --- | --- |
| Ubuntu/Linux | Build and test target | Native Fyne build target | Primary |
| macOS | Native compile target | Native Fyne compile target | CI-gated preview |
| Windows 11 | Native compile target | Native Fyne compile target | CI-gated preview |
...
Pre-1.0 desktop archives are not code-signed or notarized. macOS and Windows
may therefore show an untrusted-publisher warning until a release signing
...
[vulnerability reporting policy](SECURITY.md) before handling sensitive
targets or reporting a security issue.

## Versioning and releases

Release Please derives semantic releases from Conventional Commits and owns
release PRs, tags, GitHub Releases, and changelog updates. GoReleaser builds CLI
archives and checksums. The same release workflow packages native desktop
binaries and publishes the CLI container to GHCR after a release is created.

Local and CI development versions are computed by `scripts/version.sh`:

```text
0.1.0-dev+g9c7e123
0.1.0-dev+g9c7e123.dirty
0.1.0-pr.17.143+g9c7e123
0.1.0-dev.143+g9c7e123
```

See [versioning](docs/versioning.md) for the exact algorithm and linker flags.

## Development workflow

Contributions use Conventional Commits. Because pull requests are squash
merged, the pull request title must also use this format:
...
Before opening a pull request, run the formatting, vet, lint, unit, race, and
build checks appropriate for your platform. Tests must be deterministic and
must not contact public services. Full guidance is in
[CONTRIBUTING.md](CONTRIBUTING.md).

## Roadmap

The first release focuses on endpoint diagnostics, local history, saved
profiles, reporting, and the CLI/desktop experience. Packet capture, range
scanning, automatic network changes, remote command execution, and privilege
escalation are outside its scope.

Kubernetes service diagnosis is planned as a separate future phase using
`client-go`; that dependency and any inactive Kubernetes GUI surface are
deliberately absent from the first version. See the detailed
