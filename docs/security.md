...
For confidential vulnerability reports, follow [SECURITY.md](../SECURITY.md).
...
The published image contains only the CLI in a distroless runtime, runs as the
...
`README.md`, `CHANGELOG.md`, and `LICENSE` remain available to the builder, but
the final runtime stage copies only the compiled CLI and its non-root home
directory.
...
CI verifies module checksums, runs formatting and static analysis, unit and
race tests, `govulncheck`, native builds, and a container build. Dependabot
tracks Go modules, GitHub Actions, and Docker base images. Release Please
creates the SemVer tag and GitHub Release; assets are built from its exact
release SHA in that same workflow.

Release archives and desktop packages receive SHA-256 checksums. Linux and
macOS desktop archives use a pinned Fyne packager for native metadata and icon
resources while preserving the already version-stamped executable. The
container is published only after a release is created and is labeled with
version, commit, source, license, and build date.
