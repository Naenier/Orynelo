...
version command, GUI About screen, and stored history. Release builds inject
version, commit, and build date with linker flags. `runtime/debug.ReadBuildInfo`
supplies VCS and module fallbacks for local or `go install` builds. Nothing
rewrites a tracked Go source file during a build.
