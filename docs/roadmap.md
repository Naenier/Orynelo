...
security review, and cross-platform CI results.
...
- build metadata, native release archives, and a non-root CLI container.
...
review, and verified release smoke tests.
...
- Do not add `client-go` to the first release while no Kubernetes feature
  consumes it.
