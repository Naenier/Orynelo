...
.PHONY: run-cli run-gui docker-build docker-run snapshot clean version version-test
...
snapshot: ## Build a local GoReleaser snapshot without publishing.
	OPSDOCTOR_VERSION="$(VERSION)" $(GORELEASER) release --snapshot --clean

clean: ## Remove generated build and test output.
	rm -rf -- ./bin ./dist ./coverage.out

version: ## Print the computed build version.
	@./scripts/version.sh

version-test: ## Test the build-version algorithm.
